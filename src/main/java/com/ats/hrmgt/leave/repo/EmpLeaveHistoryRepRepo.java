...
public interface EmpLeaveHistoryRepRepo extends JpaRepository<EmpLeaveHistoryRep,String> {
	...
	@Query(value = " SELECT\n" + 
			...
			"        AND leave_structure_header.lvs_id = leave_structure_details.lvs_id \n" + 
			"        AND leave_structure_details.lv_type_id = leave_type.lv_type_id \n" + 
			"        AND emp_info.emp_id IN (SELECT emp_info.emp_id FROM emp_info WHERE emp_info.del_status=1)\n" + 
			"        and leave_structure_allotment.cal_yr_id=(SELECT dm_cal_year.cal_yr_id FROM dm_cal_year WHERE dm_cal_year.is_current=1) \n" + 
			"        and leave_type.del_status=1 ", nativeQuery = true)

	List<EmpLeaveHistoryRep> getEmpLeaveHistoryRepAll();


	@Query(value=" SELECT\n" + 
			"        CONCAT(emp_info.emp_sname,\n" + 
			"        \" \",\n" + 
			"        emp_info.emp_fname) AS emp_name,\n" + 
			"        UUID() AS rec_id,\n" + 
			"        emp_info.emp_id,\n" + 
			"        leave_type.lv_type_id,\n" + 
			"        leave_type.lv_title_short,\n" + 
			...
			"            dm_cal_year y          \n" + 
			"        where\n" + 
			"            emp_info.emp_id=:empId \n" + 
			"            and b.emp_id=emp_info.emp_id              \n" + 
			"            and leave_type.lv_type_id=b.lv_type_id              \n" + 
			"            and y.cal_yr_id=b.cal_yr_id              \n" + 
			"            and y.is_current=1),\n" + 
			"        0) as bal_leave,\n" + 
			...
			"    WHERE\n" + 
			"        emp_info.emp_id=:empId\n" + 
			"        and emp_info.emp_id = leave_structure_allotment.emp_id          \n" + 
			"        AND leave_structure_allotment.lvs_id = leave_structure_header.lvs_id          \n" + 
			"        AND leave_structure_header.lvs_id = leave_structure_details.lvs_id          \n" + 
			"        AND leave_structure_details.lv_type_id = leave_type.lv_type_id          \n" + 
			...
			"            SELECT\n" + 
			"                dm_cal_year.cal_yr_id \n" + 
			"            FROM\n" + 
			"                dm_cal_year \n" + 
			"            WHERE\n" + 
			"                dm_cal_year.is_current=1\n" + 
			"        )          \n" + 
			"        and leave_type.del_status=1 ",nativeQuery=true)
	EmpLeaveHistoryRep getEmpLeaveHistoryRep(@Param("empId") int empId);
